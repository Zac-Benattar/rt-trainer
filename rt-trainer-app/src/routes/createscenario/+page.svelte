			...
			<div class="flex flex-col px-2 xs:w-9/12 gap-2">
				...
				<div class="flex flex-col gap-1">
					...
					<div>
						...
						<ListBox class="card {routesClass}">
							{#each data.userRecentRoutes as route}
								<ListBoxItem bind:group={selectedRouteId} name="medium" value={route.id}>
									...
									<div class="flex flex-row gap-2">
										<div class="flex-col place-content-center">
											...
											{:else}
												<div class="text-sm opacity-50">{route.description?.substring(0, 200)}</div>
											{/if}
										</div>
									</div>
					...
					<div>
						...
						<input
							class="input"
							title="Weather Seed Input"
							type="text"
							placeholder={weatherSeed}
						/>
					...
					<div>
						<label class="flex items-center space-x-2 mb-3">
							<input class="checkbox" type="checkbox" checked={emergency} />

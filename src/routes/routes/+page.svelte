...
<div class="flex flex-col place-content-center">
	<div class="flex flex-row p-3 place-content-center sm:place-content-start">
		<div class="flex flex-col px-2 xs:w-9/12 gap-2">
			<span class="h3 p-1">Public Routes</span>

			<button class="btn variant-filled font-bold" on:click={() => goto('/routeplanner')}
				>Create a Route</button
			>

			{#if data.publicRoutes.length === 0}
				<div class="card p-5">
					<div class="text-center">
						<p class="text-2xl font-bold">No routes found</p>
						<p class="text-sm opacity-50">Try create your own</p>
					</div>
				</div>
			{:else}
				<nav class="list-nav">
